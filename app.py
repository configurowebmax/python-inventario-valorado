...
APP_CLAVE = "python_inventario_valorado_datos"
VERSION = "1.0.0"
...
# =====================================================================
class Calculadora:
    """Modelo de cálculo de Inventario Valorado."""
    ...
    def calcular(self):
        ...
        valor = self.cantidad * self.costo
        return {"valor": valor}


    def diagnostico(self, resultados):
        """Texto explicativo del resultado."""
        return "✅ Valor de inventario calculado."
...
def calcular_handler(event=None):
    ...
    c = Calculadora(input_float("cantidad"), input_float("costo"))
    r = c.calcular()
    html = f"""
      <div class="result-value">📦 Valor: {fmt_moneda(r["valor"])}</div>
      <p class="result-detail">{fmt_num(c.cantidad)} unidades × {fmt_moneda(c.costo)}</p>
    """
